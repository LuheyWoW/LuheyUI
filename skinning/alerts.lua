...
-- Text color
local LUI_TEXT_COLOR = { 0.953, 0.957, 0.965, 1 }
...
local function SkinAchievementAlert(frame)
    ...
    if frame.Unlocked then
        frame.Unlocked:SetTextColor(unpack(LUI_TEXT_COLOR))
    end
    if frame.Name then
...
local function SkinCriteriaAlert(frame)
    ...
    if frame.Unlocked then frame.Unlocked:SetTextColor(unpack(LUI_TEXT_COLOR)) end
...
local function CreateAlertMover()
    ...
    if not alertHolder then
        alertHolder = CreateFrame("Frame", "LUI_AlertFrameHolder", UIParent)
        ...
        alertMover = CreateFrame("Frame", "LUI_AlertFrameMover", alertHolder, "BackdropTemplate")
...
local function CreateEventToastMover()
    ...
    if not toastHolder then
        toastHolder = CreateFrame("Frame", "LUI_EventToastHolder", UIParent)
        ...
        -- Create mover overlay
        toastMover = CreateFrame("Frame", "LUI_EventToastMover", toastHolder, "BackdropTemplate")
...
_G.LuheyUI_RefreshAlertColors = RefreshAlertColors
