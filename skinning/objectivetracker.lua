...
local function HideScenarioStageArtwork()
    ...
    if stageBlock.Stage then
        ...
        -- Collapse StageBlock height to fit just the text (remove banner space)
        local stageHeight = stageBlock.Stage:GetStringHeight() or 14
        local nameHeight = stageBlock.Name and stageBlock.Name:GetStringHeight() or 16
        local totalHeight = 5 + stageHeight + 2 + nameHeight + 8  -- padding + stage + gap + name + bottom
        stageBlock:SetHeight(totalHeight)
    end
end
...
_G.LuheyUI_RefreshObjectiveTracker = RefreshObjectiveTracker
_G.LuheyUI_RefreshObjectiveTrackerColors = RefreshObjectiveTracker
