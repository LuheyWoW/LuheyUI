...
local function GetSettings()
    ...
    return LUICore.db.profile.customGlow
end
...
local function GetEffectsSettings()
    ...
    return LUICore.db.profile.cooldownEffects or { hideEssential = true, hideUtility = true }
end
...
_G.LuheyUI_RefreshCustomGlows = RefreshAllGlows

-- Debug functions
_G.LuheyUI_TestCustomGlow = function(viewerType)
    ...
    if viewer then
        ...
        for i, child in ipairs(children) do
            if child:IsShown() then
                StartGlow(child)
                print("|cFF00FF00[LuheyUI]|r Test glow applied to " .. viewerType .. " icon #" .. i)
                return
            end
        end
        print("|cFFFF0000[LuheyUI]|r No visible icons in " .. viewerType .. " viewer")
    else
        print("|cFFFF0000[LuheyUI]|r " .. viewerType .. "CooldownViewer not found")
    end
end

_G.LuheyUI_StopAllCustomGlows = function()
    for icon, _ in pairs(activeGlowIcons) do
        if icon then
            StopGlow(icon)
        end
    ...
    wipe(activeGlowIcons)
    print("|cFF00FF00[LuheyUI]|r All custom glows stopped")
end
