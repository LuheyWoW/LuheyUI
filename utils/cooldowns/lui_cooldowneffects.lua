...
local function GetSettings()
    ...
    if not LUICore.db.profile.cooldownEffects then
        LUICore.db.profile.cooldownEffects = { hideEssential = true, hideUtility = true }
    end
    return LUICore.db.profile.cooldownEffects
end
...
local function HideCooldownEffects(child)
    ...
    for _, frameName in ipairs(effectFrames) do
        ...
        if frame then
            ...
            if not frame._LuheyUI_NoShow then
                frame._LuheyUI_NoShow = true
...
local function ProcessViewer(viewerName)
        ...
        for _, child in ipairs(children) do
            if child:IsShown() then
                ...
                -- Mark as processed (no OnUpdate hook needed - we handle glows via hooksecurefunc)
                    child._LuheyUI_EffectsHidden = true
            end
        end
    end
    ...
    if viewer.Layout and not viewer._LuheyUI_EffectsHooked then
        viewer._LuheyUI_EffectsHooked = true
    ...
    -- Hook OnShow
    if not viewer._LuheyUI_EffectsShowHooked then
        viewer._LuheyUI_EffectsShowHooked = true
...
_G.LuheyUI_RefreshCooldownEffects = function()
    ApplyToAllViewers()
end
