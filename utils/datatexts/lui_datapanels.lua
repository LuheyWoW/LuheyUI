...
function Datapanels:CreatePanel(panelID, config)
    if self.activePanels[panelID] then
        print("|cffff0000LuheyUI:|r Panel '" .. panelID .. "' already exists!")
        return self.activePanels[panelID]
    end
    
    -- Create panel frame
    local panel = CreateFrame("Frame", "LUI_Datapanel_" .. panelID, UIParent)
...
function Datapanels:SetupDragging(panel)
    ...
    panel:SetScript("OnDragStop", function(self)
        ...
        local db = LUICore.db.profile.quiDatatexts
...
function Datapanels:UpdateSlots(panel)
    ...
    for i = 1, numSlots do
        ...
        if datatextID and LUICore.Datatexts then
            LUICore.Datatexts:AttachToSlot(slot, datatextID, panel.config)
        else
...
_G.LuheyUI_RefreshDatapanels = function()
...
-- Debug slash command
SLASH_LUIDATAPANELS1 = "/quidp"
SlashCmdList["LUIDATAPANELS"] = function(msg)
    if msg == "show" then
        local count = 0
        ...
        print(string.format("|cff00ff00LuheyUI:|r Total panels: %d", count))
    elseif msg == "refresh" then
        Datapanels:RefreshAll()
        print("|cff00ff00LuheyUI:|r Refreshed all datapanels")
    else
        print("|cff00ff00LuheyUI Datapanels Commands:|r")
