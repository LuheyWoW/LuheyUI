...
local function GetLockoutCacheTTL()
    local db = LUICore.db and LUICore.db.profile and LUICore.db.profile.datatext
...
function Datatexts:Register(id, datatextDef)
    if self.registry[id] then
        print("|cffff0000LuheyUI:|r Datatext '" .. id .. "' is already registered!")
        return false
    end
    
    if not datatextDef.OnEnable or type(datatextDef.OnEnable) ~= "function" then
        print("|cffff0000LuheyUI:|r Datatext '" .. id .. "' missing OnEnable function!")
        return false
    end
...
function Datatexts:AttachToSlot(slotFrame, datatextID, settings)
    if not slotFrame then
        print("|cffff0000LuheyUI:|r Invalid slot frame provided")
        return false
    end
    ...
    if not success then
        print("|cffff0000LuheyUI:|r Failed to enable datatext '" .. datatextID .. "': " .. tostring(instance))
        return false
    end
...
Datatexts:Register("time", {
    ...
    OnEnable = function(slotFrame, settings)
        ...
        local function Update()
            ...
            local dtSettings = LUICore.db and LUICore.db.profile and LUICore.db.profile.datatext
                    ...
                    local btn = root:CreateButton(colorCode .. charKey .. "|r - " .. FormatGold(charMoney), function()
                        ...
                        StaticPopupDialogs["QUAZII_GOLD_DELETE_CHAR"] = {
                            ...
                            OnAccept = function()
                                ...
                                print("|cff30D1FF[LuheyUI]|r Removed gold data for " .. deleteCharKey)
                            end,
                            timeout = 0,
                            whileDead = true,
                            hideOnEscape = true,
                ...
                root:CreateButton("|cffFF6666Reset All (Keep Current)|r", function()
                    StaticPopupDialogs["QUAZII_GOLD_RESET_ALL"] = {
                        ...
                        OnAccept = function()
                            ...
                            print("|cff30D1FF[LuheyUI]|r Reset gold data (kept current character)")
                        end,
                        timeout = 0,
                        whileDead = true,
                        hideOnEscape = true,
...
local function GetShortDungeonName(mapID)
    if _G.LUI_DungeonData then
        return _G.LUI_DungeonData.GetShortName(mapID)
    end
                            ...
                            if specName then
                                ...
                                root:CreateButton(iconText .. " " .. specName .. (isActive and activeMarker or ""), function()
                                    if InCombatLockdown() then
                                        print("|cffFF6B6BLuheyUI:|r Cannot change specialization in combat")
                                        return
                                    end
