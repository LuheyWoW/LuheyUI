...
local function IsNudgeTargetFrameName(frameName)
    ...
    if LUICore.viewers then
        for _, viewerName in ipairs(LUICore.viewers) do
...
local function CreateNudgeUI()
    ...
    local function CreateArrowButton(parent, direction, x, yFromTop)
        ...
        button:SetScript("OnClick", function()
            LUICore:NudgeSelectedViewer(direction)
    ...
    amountSlider:SetScript("OnValueChanged", function(self, value)
        ...
        LUICore.db.profile.nudgeAmount = value
    ...
    local function ViewerDropdown_Initialize(self, level)
        ...
        if LUICore.viewers then
            for _, viewerName in ipairs(LUICore.viewers) do
                ...
                info.func = function()
                    LUICore:SelectViewer(viewerName)
                    UIDropDownMenu_SetText(viewerDropdown, displayName)
                    CloseDropDownMenus()
                end
                info.checked = (LUICore.selectedViewer == viewerName)
        ...
        for frameName, label in pairs(UNIT_ANCHOR_FRAMES) do
            ...
            info.func = function()
                LUICore:SelectViewer(frameName)
                UIDropDownMenu_SetText(viewerDropdown, displayName)
                CloseDropDownMenus()
            end
            info.checked = (LUICore.selectedViewer == frameName)
            UIDropDownMenu_AddButton(info, level)
        end
    end
    ...
    function NudgeFrame:UpdateInfo()
        local viewerName = LUICore.selectedViewer
    ...
    function NudgeFrame:UpdateAmountSlider()
        local amount = LUICore.db.profile.nudgeAmount or 1
...
local function CreateMinimapNudgeButton(parent, direction)
    ...
    btn:SetScript("OnClick", function()
        LUICore:SelectEditModeElement("minimap", "minimap")
        LUICore:NudgeMinimap(direction)
    end)
...
function LUICore:ShowViewerOverlays()
...
function LUICore:HideViewerOverlays()
    for _, viewerName in ipairs(CDM_VIEWERS) do
        local overlay = viewerOverlays[viewerName]
        if overlay then
            overlay:Hide()
...
function LUICore:ShowBlizzardFrameOverlays()
...
function LUICore:HideBlizzardFrameOverlays()
    for _, frameInfo in ipairs(BLIZZARD_EDITMODE_FRAMES) do
...
function LUICore:ShowMinimapOverlay()
    ...
    if minimapOverlay then
        ...
        minimapOverlay:SetScript("OnMouseUp", function(self, button)
            ...
            -- Save position to DB
            local settings = LUICore.db and LUICore.db.profile and LUICore.db.profile.minimap
            if settings then
...
function LUICore:HideMinimapOverlay()
    if minimapOverlay then
        minimapOverlay:Hide()
...
function LUICore:EnableClickDetection()
    clickDetector:Show()
                    ...
                    if frame and frame ~= WorldFrame then
                        ...
                        if not frameName and frame:GetParent() then
                            ...
                            if IsNudgeTargetFrameName(parentName) then
                                if lastClickedFrame ~= frame then
                                    lastClickedFrame = frame
                                    LUICore:SelectViewer(parentName)
                                end
                                return
                            end
                        end
...
function LUICore:DisableClickDetection()
    clickDetector:Hide()
...
function LUICore:SelectViewer(viewerName)
...
function LUICore:NudgeSelectedViewer(direction)
...
-- Nudge the minimap
function LUICore:NudgeMinimap(direction)
...
local function SetupEditModeHooks()
    ...
    hooksecurefunc(EditModeManagerFrame, "EnterEditMode", function()
        ...
        if LUICore.nudgeFrame then
            LUICore.nudgeFrame:UpdateVisibility()
        end
        LUICore:EnableClickDetection()
        -- Let Blizzard's native Edit Mode handle CDM viewers and standard Edit Mode frames
        -- LUICore:ShowViewerOverlays()
        -- LUICore:ShowBlizzardFrameOverlays()
        LUICore:ShowMinimapOverlay()  -- Show nudge overlay on QUI minimap
        LUICore:EnableMinimapEditMode()  -- Temporarily allow minimap movement
    end)

    hooksecurefunc(EditModeManagerFrame, "ExitEditMode", function()
        -- NudgeFrame is lazy-loaded, only hide if it exists
        if LUICore.nudgeFrame then
            LUICore.nudgeFrame:Hide()
        end
        LUICore:DisableClickDetection()
        -- LUICore:HideViewerOverlays()
        -- LUICore:HideBlizzardFrameOverlays()
        LUICore:HideMinimapOverlay()  -- Hide minimap overlay
        LUICore:DisableMinimapEditMode()  -- Restore minimap lock setting
        LUICore.selectedViewer = nil
        -- Clear central selection (in case a CDM viewer was selected)
        if LUICore.ClearEditModeSelection then
...
-- Add nudgeamount
local oldOnInitialize = LUICore.OnInitialize
