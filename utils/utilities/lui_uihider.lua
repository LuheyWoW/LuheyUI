...
local function GetSettings()
    ...
    if not LUICore or not LUICore.db or not LUICore.db.profile then
        return nil
    end
    -- Ensure uiHider table exists
    if not LUICore.db.profile.uiHider then
        LUICore.db.profile.uiHider = {
    ...
    local uiHider = LUICore.db.profile.uiHider
...
local function ApplyHideSettings()
    ...
    if ObjectiveTrackerFrame then
        ...
        if shouldHide then
            ...
            if not ObjectiveTrackerFrame._LUI_ShowHooked then
                ObjectiveTrackerFrame._LUI_ShowHooked = true
    ...
    if GameTimeFrame then
        ...
        if not GameTimeFrame._LUI_ShowHooked then
            GameTimeFrame._LUI_ShowHooked = true
    ...
    if CompactRaidFrameManager then
        ...
        elseif settings.hideRaidFrameManager then
            ...
            if not CompactRaidFrameManager._LUI_ShowHooked then
                CompactRaidFrameManager._LUI_ShowHooked = true
            ...
            -- BUG-008: Wrap in C_Timer.After(0) to break taint chain from secure Blizzard code
            if not CompactRaidFrameManager._LUI_SetShownHooked then
                CompactRaidFrameManager._LUI_SetShownHooked = true
    ...
    if BuffFrame and BuffFrame.CollapseAndExpandButton then
        ...
        if settings.hideBuffCollapseButton then
            ...
            if not btn._LUI_AlphaHooked then
                btn._LUI_AlphaHooked = true
    ...
    if TalkingHeadFrame then
        ...
        if settings.hideTalkingHead then
            ...
            if not TalkingHeadFrame._LUI_ShowHooked then
                TalkingHeadFrame._LUI_ShowHooked = true
            ...
            if not TalkingHeadFrame._LUI_MouseManaged then
                TalkingHeadFrame._LUI_MouseManaged = true
        ...
        if not TalkingHeadFrame._LUI_MuteHooked then
            TalkingHeadFrame._LUI_MuteHooked = true
            hooksecurefunc(TalkingHeadFrame, "PlayCurrent", function()
                local s = GetSettings()
    ...
    if StatusTrackingBarManager then
        ...
        if hideXP and hideRep then
            ...
            if not StatusTrackingBarManager._LUI_ShowHooked then
                StatusTrackingBarManager._LUI_ShowHooked = true
        ...
        elseif hideXP or hideRep then
            ...
            if not StatusTrackingBarManager._LUI_BarsHooked then
                StatusTrackingBarManager._LUI_BarsHooked = true
    ...
    if WorldMapFrame and WorldMapFrame.BlackoutFrame then
        if settings.hideWorldMapBlackout then
            ...
            if not WorldMapFrame.BlackoutFrame._LUI_BlackoutHooked then
                WorldMapFrame.BlackoutFrame._LUI_BlackoutHooked = true
...
_G.LuheyUI_RefreshUIHider = function()
    ApplyHideSettings()
end
