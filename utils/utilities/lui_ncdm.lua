...
local function GetGeneralFont()
    if LUICore and LUICore.db and LUICore.db.profile and LUICore.db.profile.general then
        local general = LUICore.db.profile.general
...
local function GetGeneralFontOutline()
    if LUICore and LUICore.db and LUICore.db.profile and LUICore.db.profile.general then
        return LUICore.db.profile.general.fontOutline or "OUTLINE"
    end
    return "OUTLINE"
end
...
local function GetDB()
    if LUICore and LUICore.db and LUICore.db.profile and LUICore.db.profile.ncdm then
        return LUICore.db.profile.ncdm
    end
    return nil
end
...
combatEndFrame:SetScript("OnEvent", function()
    ...
    C_Timer.After(0.1, function()
        if not InCombatLockdown() and _G.LuheyUI_RefreshNCDM then
...
local function LayoutViewer(viewerName, trackerKey)
    ...
    if not viewer.__cdmUpdatePending then
        ...
        C_Timer.After(0.05, function()
            ...
            if trackerKey == "essential" then
                if _G.LuheyUI_UpdateLockedPowerBar then
                    _G.LuheyUI_UpdateLockedPowerBar()
                end
                if _G.LuheyUI_UpdateLockedSecondaryPowerBar then
                    _G.LuheyUI_UpdateLockedSecondaryPowerBar()
                end
                if _G.LuheyUI_UpdateLockedCastbarToEssential then
                    _G.LuheyUI_UpdateLockedCastbarToEssential()
                end
            elseif trackerKey == "utility" then
                if _G.LuheyUI_UpdateLockedPowerBarToUtility then
                    _G.LuheyUI_UpdateLockedPowerBarToUtility()
                end
                if _G.LuheyUI_UpdateLockedSecondaryPowerBarToUtility then
                    _G.LuheyUI_UpdateLockedSecondaryPowerBarToUtility()
                end
                if _G.LuheyUI_UpdateLockedCastbarToUtility then
                    _G.LuheyUI_UpdateLockedCastbarToUtility()
                end
            end
            -- Unit frames anchored to CDM (can be anchored to either Essential or Utility)
            if _G.LuheyUI_UpdateCDMAnchoredUnitFrames then
                _G.LuheyUI_UpdateCDMAnchoredUnitFrames()
            end
            -- Update keybind text on CDM icons
            if _G.LuheyUI_UpdateViewerKeybinds then
                _G.LuheyUI_UpdateViewerKeybinds(viewerName)
            end
        end)
    end
end
    ...
    viewer:HookScript("OnShow", function(self)
        ...
        C_Timer.After(0.02, function()
            if self:IsShown() then
                ...
                if trackerKey == "utility" and _G.LuheyUI_ApplyUtilityAnchor then
                    _G.LuheyUI_ApplyUtilityAnchor()
                end
            end
        end)
...
_G.LuheyUI_RefreshNCDM = RefreshAll
_G.LuheyUI_IncrementNCDMVersion = IncrementSettingsVersion
_G.LuheyUI_ApplyUtilityAnchor = ApplyUtilityAnchor
...
local function GetCDMFrames()
    ...
    -- LuheyUI power bars - always include in CDM visibility control
    -- (standalone mode only affects positioning, not visibility)
    if LUICore then
        if LUICore.powerBar then
            table.insert(frames, LUICore.powerBar)
        end
        if LUICore.secondaryPowerBar then
            table.insert(frames, LUICore.secondaryPowerBar)
        end
    end
...
local function GetCDMVisibilitySettings()
    if LUICore and LUICore.db and LUICore.db.profile and LUICore.db.profile.cdmVisibility then
        return LUICore.db.profile.cdmVisibility
    end
    return nil
end
...
local function GetUnitframesVisibilitySettings()
    if LUICore and LUICore.db and LUICore.db.profile and LUICore.db.profile.unitframesVisibility then
        return LUICore.db.profile.unitframesVisibility
    end
    return nil
end
...
local function GetUnitframeFrames()
    ...
    -- Collect unit frames
    if _G.LuheyUI_UnitFrames then
        for unitKey, frame in pairs(_G.LuheyUI_UnitFrames) do
            if frame then
    ...
    if not (vis and vis.alwaysShowCastbars) then
        if _G.LuheyUI_Castbars then
            for unitKey, castbar in pairs(_G.LuheyUI_Castbars) do
                if castbar then
...
local function UpdateUnitframesVisibility()
    ...
    if _G.LuheyUI_Castbars then
        ...
        if vis and vis.alwaysShowCastbars then
            ...
            if _G.LuheyUI_UnitFrames then
                for _, frame in pairs(_G.LuheyUI_UnitFrames) do
                    if frame then
...
_G.LuheyUI_RefreshCDMVisibility = UpdateCDMVisibility
_G.LuheyUI_RefreshUnitframesVisibility = UpdateUnitframesVisibility
_G.LuheyUI_RefreshCDMMouseover = SetupCDMMouseoverDetector
_G.LuheyUI_RefreshUnitframesMouseover = SetupUnitframesMouseoverDetector
