...
local function IsAnyKeybindFeatureEnabled()
    ...
    if not LUICore or not LUICore.db or not LUICore.db.profile then return false end

    local viewers = LUICore.db.profile.viewers
...
local function ApplyKeybindToIcon(icon, viewerName)
    local LUICore = _G.LuheyUI and _G.LuheyUI.LUICore
    if not LUICore or not LUICore.db or not LUICore.db.profile then return end
    
    local settings = LUICore.db.profile.viewers[viewerName]
...
local function HookViewerLayout(viewerName)
    ...
    if viewer.Layout and not viewer._LUI_KeybindHooked then
        viewer._LUI_KeybindHooked = true
...
_G.LuheyUI_UpdateViewerKeybinds = function(viewerName)
    -- PERFORMANCE: Skip if no keybind features are enabled
    if not IsAnyKeybindFeatureEnabled() then return end
    UpdateViewerKeybinds(viewerName)
end
...
SLASH_LUIKEYBINDS1 = "/quikeybinds"
SlashCmdList["LUIKEYBINDS"] = function(msg)
    if msg == "debug" then
        RebuildCache()
        DebugPrintCache()
...
_G.LuheyUI_RefreshKeybinds = UpdateAllKeybinds
_G.LuheyUI_RefreshRotationHelper = RefreshRotationHelper
