...
local ADDON_NAME, ns = ...
local QUI = LuheyUI  -- Use global addon table, not ns.Addon (which is LUICore)
...
-- Will be set during initialization when LUICore is available
local LUICore
...
local function GetRechargeEdgeSetting()
    local core = _G.LuheyUI and _G.LuheyUI.LUICore
...
local function GetDB()
    if LUICore and LUICore.db and LUICore.db.profile and LUICore.db.profile.customTrackers then
        return LUICore.db.profile.customTrackers
    end
    return nil
end

local function GetGlobalDB()
    if LUICore and LUICore.db and LUICore.db.global then
        return LUICore.db.global
    end
    return nil
end
...
local PROC_GLOW_MASK = "Interface\\AddOns\\LuheyUI\\assets\\iconskin\\ProcGlowMask"
...
local function CreateTrackerIcon(parent)
    ...
    icon:SetScript("OnEnter", function(self)
        ...
        if self.entry then
            ...
            local tooltipSettings = QUI.LUICore and QUI.LUICore.db and QUI.LUICore.db.profile and QUI.LUICore.db.profile.tooltip
...
function CustomTrackers:CreateBar(barID, config)
    ...
    local bar = CreateFrame("Frame", "LUI_CustomTracker_" .. barID, UIParent, "BackdropTemplate")
    ...
    if LUICore and LUICore.GetHUDFrameLevel then
        frameLevel = LUICore:GetHUDFrameLevel(layerPriority)
    end
...
---------------------------------------------------------------------------
_G.LuheyUI_RefreshCustomTrackers = function()
    -- Use local CustomTrackers directly since LUICore might not be set yet
...
initFrame:SetScript("OnEvent", function(self, event, ...)
    ...
    if event == "PLAYER_ENTERING_WORLD" then
        -- Set LUICore reference now that it's available
        LUICore = QUI.LUICore
        if LUICore then
            LUICore.CustomTrackers = CustomTrackers
        end
...
local function GetCustomTrackersVisibilitySettings()
    local db = QUI.LUICore and QUI.LUICore.db
...
---------------------------------------------------------------------------
_G.LuheyUI_RefreshCustomTrackersVisibility = UpdateCustomTrackersVisibility
_G.LuheyUI_RefreshCustomTrackersMouseover = SetupCustomTrackersMouseoverDetector

-- Refresh keybind display on all custom tracker icons
_G.LuheyUI_RefreshCustomTrackerKeybinds = function()
