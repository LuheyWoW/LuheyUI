...
local function GetDefaultTexture()
    if LUICore and LUICore.db and LUICore.db.profile and LUICore.db.profile.general then
        return LUICore.db.profile.general.texture or "Quazii"
    end
    return "Quazii"
end
...
function LUICore:RegisterPowerBarEditModeSliders(barKey, xSlider, ySlider)
...
function LUICore:NotifyPowerBarPositionChanged(barKey, offsetX, offsetY)
...
local function GetResourceColor(resource)
    ...
    local pc = LUICore and LUICore.db and LUICore.db.profile.powerColors
...
local function CreatePowerBarNudgeButton(parent, direction, deltaX, deltaY, barKey)
    ...
    btn:SetScript("OnClick", function()
        local cfg = (barKey == "primary") and LUICore.db.profile.powerBar or LUICore.db.profile.secondaryPowerBar
        ...
        LUICore:NotifyPowerBarPositionChanged(barKey, cfg.offsetX, cfg.offsetY)
    end)
...
function LUICore:EnablePowerBarEditMode()
            ...
            bar:SetScript("OnMouseDown", function(self, button)
                if button == "LeftButton" and PowerBarEditMode.active then
                    if LUICore and LUICore.SelectEditModeElement then
                        LUICore:SelectEditModeElement("powerbar", self._editModeBarKey)
                    end
                end
            end)
                ...
                if PowerBarEditMode.active then
                    ...
                    self:SetScript("OnUpdate", function(frame)
                        ...
                        if selfX and selfY and parentX and parentY then
                            ...
                            -- Notify options panel
                            LUICore:NotifyPowerBarPositionChanged(barKey, offsetX, offsetY)
                        end
                    end)
                end
            end)
...
function LUICore:DisablePowerBarEditMode()
...
function LUICore:UpdatePowerBar()
    local cfg = self.db.profile.powerBar
...
function LUICore:UpdatePowerBarTicks(bar, resource, max)
    local cfg = self.db.profile.powerBar
...
_G.LuheyUI_UpdateLockedPowerBar = function()
    local LUICore = _G.LuheyUI and _G.LuheyUI.LUICore
    if not LUICore or not LUICore.db then return end

    local cfg = LUICore.db.profile.powerBar
...
_G.LuheyUI_UpdateLockedPowerBarToUtility = function()
    local LUICore = _G.LuheyUI and _G.LuheyUI.LUICore
    if not LUICore or not LUICore.db then return end
...
_G.LuheyUI_UpdateLockedSecondaryPowerBar = function()
    local LUICore = _G.LuheyUI and _G.LuheyUI.LUICore
    if not LUICore or not LUICore.db then return end

    local cfg = LUICore.db.profile.secondaryPowerBar
    if not cfg.enabled or not cfg.lockedToEssential then return end
...
_G.LuheyUI_UpdateLockedSecondaryPowerBarToUtility = function()
    local LUICore = _G.LuheyUI and _G.LuheyUI.LUICore
    if not LUICore or not LUICore.db then return end
...
function LUICore:GetSecondaryPowerBar()
...
function LUICore:CreateFragmentedPowerBars(bar, resource, isVertical)
    local cfg = self.db.profile.secondaryPowerBar
...
function LUICore:UpdateFragmentedPowerDisplay(bar, resource, isVertical)
    local cfg = self.db.profile.secondaryPowerBar
    local maxPower = UnitPowerMax("player", resource)
...
function LUICore:UpdateSecondaryPowerBarTicks(bar, resource, max)
    local cfg = self.db.profile.secondaryPowerBar
...
function LUICore:UpdateSecondaryPowerBar()
    local cfg = self.db.profile.secondaryPowerBar
    if not cfg.enabled then
...
function LUICore:OnUnitPower(_, unit)
...
local oldRefreshAll = LUICore.RefreshAll
...
function LUICore:OnRunePowerUpdate()
    local now = GetTime()
...
local function InitializeResourceBars(self)
    ...
    C_Timer.After(0.6, function()
        if EditModeManagerFrame and not LUICore._powerBarEditModeHooked then
            LUICore._powerBarEditModeHooked = true
            hooksecurefunc(EditModeManagerFrame, "EnterEditMode", function()
                if not InCombatLockdown() then
                    LUICore:EnablePowerBarEditMode()
                end
            end)
            hooksecurefunc(EditModeManagerFrame, "ExitEditMode", function()
                if not InCombatLockdown() then
                    LUICore:DisablePowerBarEditMode()
                end
            end)
        end
    end)
...
function LUICore:OnSpecChanged()
...
function LUICore:OnShapeshiftChanged()
    -- Druid form changes affect primary/secondary resources
    self:UpdatePowerBar()
    self:UpdateSecondaryPowerBar()
end
-- Hook into that shit
local oldOnEnable = LUICore.OnEnable
