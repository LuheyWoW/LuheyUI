...
LUICore.Minimap = Minimap_Module
...
-- Datatext panel (3-slot architecture using LUICore.Datatexts registry)
local datatextFrame
...
local function GetSettings()
    ...
    cachedSettings = LUICore.db.profile.minimap
...
local function CreateBackdrop()
    ...
    backdropFrame = CreateFrame("Frame", "LUI_MinimapBackdrop", Minimap)
...
local function GetDatatextSettings()
    ...
    return LUICore.db.profile.datatext
end
...
local function CreateDatatextPanel()
    ...
    datatextFrame = CreateFrame("Frame", "LUI_DatatextPanel", UIParent)
...
local function RefreshDatatextSlots()
    ...
    if not LUICore or not LUICore.Datatexts then return end
    ...
    for i, slot in ipairs(datatextFrame.slots) do
        ...
        if slot.datatextInstance then
            LUICore.Datatexts:DetachFromSlot(slot)
        end

        -- Apply font to ALL slots (prevents "Font not set" error on empty slots)
        LUICore:SafeSetFont(slot.text, fontPath, fontSize, generalOutline)
        ...
        if datatextID and datatextID ~= "" then
            ...
            -- Attach datatext
            LUICore.Datatexts:AttachToSlot(slot, datatextID, dtSettings)
        else
            -- Empty slot: hide
            slot:Hide()
            slot.text:SetText("")
...
local function UpdateClock()
    ...
    LUICore:SafeSetFont(clockText, fontPath, clockConfig.fontSize, flags)
...
local function UpdateCoords()
    ...
    LUICore:SafeSetFont(coordsText, fontPath, coordsConfig.fontSize, flags)
...
local function UpdateZoneText()
    ...
    local fontPath = LSM:Fetch("font", generalFont) or "Fonts\\FRIZQT__.TTF"
    LUICore:SafeSetFont(zoneTextFont, fontPath, zoneConfig.fontSize, flags)
...
if Minimap.ZoomIn and not Minimap.ZoomIn._LUI_ShowHooked then
    Minimap.ZoomIn._LUI_ShowHooked = true
...
if Minimap.ZoomOut and not Minimap.ZoomOut._LUI_ShowHooked then
    Minimap.ZoomOut._LUI_ShowHooked = true
...
function LUICore:EnableMinimapEditMode()
    local settings = GetSettings()
...
function LUICore:DisableMinimapEditMode()
    local settings = GetSettings()
    if not settings then return end
