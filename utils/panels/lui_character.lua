...
-- Color palette (LuheyUI brand colors)
local C = {
...
local function GetGlobalFont()
    ...
    if LUICore and LUICore.db and LUICore.db.profile then
        local fontName = LUICore.db.profile.general and LUICore.db.profile.general.font
        if fontName then
...
local function IsSkinningHandlingBackground()
    local skinningAPI = _G.LUI_CharacterFrameSkinning
...
---------------------------------------------------------------------------
-- Create custom LuheyUI background
---------------------------------------------------------------------------
local function CreateCustomBackground()
    ...
    if skinningAPI and skinningAPI.IsEnabled and skinningAPI.IsEnabled() then
        ...
        if not customBg then
            customBg = CreateFrame("Frame", "LUI_CharacterFrameBg_CharPane", CharacterFrame, "BackdropTemplate")
...
_G.LuheyUI_RefreshCharacterPanelFonts = RefreshCharacterPanelFonts
...
local function UpdateStatsPanel(panel, unit)
    ...
    if not success then
        print("LuheyUI: Error updating stats panel:", err)
    end
end
...
local function CreateEquipMgrPopup()
    ...
    local PANEL_WIDTH_EXTENSION = 55
    equipMgrPopup = CreateFrame("Frame", "LuheyUI_EquipMgrPopup", UIParent, "BackdropTemplate")
    ...
    _G.LuheyUI_EquipMgrPopup = equipMgrPopup
...
local function CreateTitlesPopup()
    ...
    -- Position accounts for extended character pane (55px width extension + 10px gap)
    local PANEL_WIDTH_EXTENSION = 55
    titlesPopup = CreateFrame("Frame", "LuheyUI_TitlesPopup", UIParent, "BackdropTemplate")
    ...
    -- Expose globally for skinning module to access
    _G.LuheyUI_TitlesPopup = titlesPopup
...
local function HookCharacterFrame()
    ...
    if not CharacterFrame._quiGearBtn then
        gearBtn = CreateFrame("Button", "LUI_CharacterSettingsBtn", CharacterFrame, "BackdropTemplate")
        ...
        settingsPanel = CreateFrame("Frame", "LuheyUI_CharSettingsPanel", CharacterFrame, "BackdropTemplate")
        ...
        title:SetText("LUI Character Panel")
        ...
        local GUI = _G.LuheyUI and _G.LuheyUI.GUI
        ...
        local function RefreshAll()
            if _G.LuheyUI_RefreshCharacterPanelFonts then
                _G.LuheyUI_RefreshCharacterPanelFonts()
            end
            ScheduleUpdate()
        end
        ...
        local LUICore = _G.LuheyUI and _G.LuheyUI.LUICore
        local generalDB = LUICore and LUICore.db and LUICore.db.profile and LUICore.db.profile.general
...
---------------------------------------------------------------------------
_G.LuheyUI_RefreshCharacterPane = function()
    ScheduleUpdate()
end
