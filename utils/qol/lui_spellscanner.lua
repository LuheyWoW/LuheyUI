...
---------------------------------------------------------------------------
-- DATABASE ACCESS
-- Uses LuheyUI.db.global.spellScanner for cross-character persistence
---------------------------------------------------------------------------
    ...
    if bestMatch then
        ...
        if success then
            ...
            if SpellScanner.scanMode then
                print(string.format("|cff00ff00LuheyUI:|r Scanned: %s = %.1fs",
...
SLASH_LUISCAN1 = "/quiscan"
SlashCmdList["LUISCAN"] = function()
    local enabled = SpellScanner.ToggleScanMode()
    if enabled then
        print("|cff00ff00LuheyUI:|r Scan mode |cff00ff00ENABLED|r")
        print("|cffff8800-|r Cast abilities to scan their durations")
        print("|cffff8800-|r Type /quiscan again to stop")
    else
        print("|cff00ff00LuheyUI:|r Scan mode |cffff0000DISABLED|r")
    end
end

-- /quiscanned - List scanned spells
SLASH_LUISCANNED1 = "/quiscanned"
SlashCmdList["LUISCANNED"] = function()
    local db = GetDB()
    if not db then
        print("|cffff0000LuheyUI:|r Database not available")
        return
    end

    print("|cff00ff00LuheyUI Scanned Spells:|r")
    local spellCount = 0
    ...
    print("|cff00ff00LuheyUI Scanned Items:|r")
    local itemCount = 0
...
SLASH_LUICLEARSPELL1 = "/quiclearspell"
SlashCmdList["LUICLEARSPELL"] = function(msg)
    local spellID = tonumber(msg:trim())
    if not spellID then
        print("|cffff0000LuheyUI:|r Usage: /quiclearspell <spellID>")
        return
    end
    ...
    if db and db.spells and db.spells[spellID] then
        ...
        print(string.format("|cff00ff00LuheyUI:|r Cleared spell: %s [%d]", name, spellID))
    else
        print(string.format("|cffff8800LuheyUI:|r Spell %d not found in scanned list", spellID))
    end
end
