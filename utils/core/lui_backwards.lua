...
function LuheyUI:BackwardsCompat()
    ...
    if LuheyUI_DB and LuheyUI_DB.profiles and LuheyUI_DB.profiles.Default then
        self:DebugPrint("Profiles.Default.imports Exists: " .. tostring(not (not LuheyUI_DB.profiles.Default.imports)))
        ...
        if LuheyUI_DB.profiles.Default.imports and (not self.db.global.imports or next(self.db.global.imports) == nil) then
            self:DebugPrint("Import Data found in profile imports but not global imports.")
            self.db.global.imports = LuheyUI_DB.profiles.Default.imports
        end
    end
end
