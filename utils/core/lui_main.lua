--- LuheyUI Core - Main Integration Module
--- Integrated from LUICore v1.18 with proper AceDB support
--- All branding changed to LuheyUI

local ADDON_NAME, ns = ...
local QUI = LuheyUI
local ADDON_NAME = "LuheyUI"

-- Create LUICore as an Ace3 module within LuheyUI
local LUICore = QUI:NewModule("LUICore", "AceConsole-3.0", "AceEvent-3.0")
QUI.LUICore = LUICore

-- Expose LUICore to namespace for other files
ns.Addon = LUICore
...
LUICore.__pendingReload = false
LUICore.__reloadEventFrame = nil

-- Safe reload function - queues if in combat, reloads immediately if not
function LUICore:SafeReload()
    if InCombatLockdown() then
        if not self.__pendingReload then
            self.__pendingReload = true
            print("|cFF30D1FFLuhey UI:|r Reload queued - will execute when combat ends.")
            ...
            if not self.__reloadEventFrame then
                ...
                self.__reloadEventFrame:SetScript("OnEvent", function(frame, event)
                    if event == "PLAYER_REGEN_ENABLED" and LUICore.__pendingReload then
                        LUICore.__pendingReload = false
                        -- Show popup with reload button (user click = allowed)
                        LUICore:ShowReloadPopup()
                    end
                end)
            end
        end
...
function LUICore:ShowReloadPopup()
    -- Use LuheyUI's existing confirmation dialog
    if LuheyUI and LuheyUI.GUI and LuheyUI.GUI.ShowConfirmation then
        LuheyUI.GUI:ShowConfirmation({
            title = "Reload Ready",
        ...
        print("|cFF30D1FFLuhey UI:|r Combat ended. Type /reload to reload.")
    end
end

-- Global safe reload function on LuheyUI object
function QUI:SafeReload()
    if self.LUICore then
        self.LUICore:SafeReload()
    else
        -- Fallback if LUICore not loaded
        if InCombatLockdown() then
            print("|cFF30D1FFLuhey UI:|r Cannot reload during combat.")
        else
            ReloadUI()
        end
    end
...
function LUICore:ExportProfileToString()
...
function LUICore:ImportProfileFromString(str)
    if not self.db or not self.db.profile then
    ...
    str = str:gsub("%s+", "")
    str = str:gsub("^LUI1:", "")  -- LuheyUI prefix
    str = str:gsub("^LUI1:", "")  -- Strip QUI1 prefix
...
function LUICore:GetHUDFrameLevel(priority)
...
function LUICore.SafeSetBackdrop(frame, backdropInfo, borderColor)
    ...
    if not hasValidSize then
        ...
        LUICore.__pendingBackdrops = LUICore.__pendingBackdrops or {}
        LUICore.__pendingBackdrops[frame] = true

        -- Set up deferred processing via OnUpdate (for when dimensions become valid)
        if not LUICore.__backdropUpdateFrame then
            ...
            updateFrame:SetScript("OnUpdate", function(self, delta)
                ...
                local processed = {}
                for pendingFrame in pairs(LUICore.__pendingBackdrops or {}) do
                ...
                for _, pf in ipairs(processed) do
                    LUICore.__pendingBackdrops[pf] = nil
                end

                -- Stop OnUpdate if no more pending
                local hasAny = false
                for _ in pairs(LUICore.__pendingBackdrops or {}) do
                    hasAny = true
                    break
                end
                if not hasAny then
            ...
            LUICore.__backdropUpdateFrame = updateFrame
        end
        LUICore.__backdropUpdateFrame:Show()
        return false
    end

    -- If in combat, defer backdrop setup to avoid secret value errors
    if InCombatLockdown() then
        local alreadyPending = LUICore.__pendingBackdrops and LUICore.__pendingBackdrops[frame]
        if not alreadyPending then
            frame.__quiBackdropPending = backdropInfo
            frame.__quiBackdropBorderColor = borderColor

            if not LUICore.__backdropEventFrame then
                ...
                eventFrame:SetScript("OnEvent", function(self)
                    ...
                    LUICore.__pendingBackdrops = {}
                end)
                LUICore.__backdropEventFrame = eventFrame
            end

            LUICore.__pendingBackdrops = LUICore.__pendingBackdrops or {}
            LUICore.__pendingBackdrops[frame] = true
            LUICore.__backdropEventFrame:RegisterEvent("PLAYER_REGEN_ENABLED")
        end
        return false
    end
...
local defaults = {
    profile = {
        ...
        general = {
            ...
            addLuheyUIButton = false,  -- Add Luhey UI button to ESC menu (opt-in)
            ...
            skinBagFrames = true,  -- Skin bag/container frames
        ...
        character = {
            ...
            inspectLiteFont = "Quazii",
            inspectLiteOutline = "OUTLINE",
            -- Overall iLvL display settings
            inspectLiteOverallFont = "Quazii",
        ...
        tooltip = {
            ...
            -- Visual Styling
            scale = 1.0,                       -- Global tooltip scale (0.5-1.5)
            backgroundColor = {0.05, 0.05, 0.05, 0.95},  -- Tooltip backdrop RGBA
            borderColor = {0.2, 1.0, 0.6, 1.0},          -- LUI mint accent border
            classColoredBorder = false,        -- Border matches player class (players only)
            reactionColoredBorder = false,     -- Border based on unit reaction

            -- Information Display
            showTarget = false,                -- Show unit's target with <<YOU>> indicator
            targetYouText = "<<YOU>>",         -- Text displayed when targeting player
            showGuildRank = false,             -- Show guild rank alongside guild name
            realmDisplay = "crossrealm",       -- "always", "never", "crossrealm"
            realmFormat = "full",              -- "full" or "asterisk"
            showMount = false,                 -- Show mount name when player is mounted
            showRaiderIO = false,              -- Show M+ score if Raider.IO installed
        },
...
function LUICore:OnInitialize()
    self.db = LibStub("AceDB-3.0"):New("LuheyUIDB", defaults, true)
...
function LUICore:OnProfileChanged(event, db, profileKey)
    ...
    local function ApplyUIScale(scale)
        if InCombatLockdown() then
            LUICore._pendingUIScale = scale
            if not LUICore._scaleRegenFrame then
                LUICore._scaleRegenFrame = CreateFrame("Frame")
                LUICore._scaleRegenFrame:SetScript("OnEvent", function(self)
                    self:UnregisterEvent("PLAYER_REGEN_ENABLED")
                    if LUICore._pendingUIScale and not InCombatLockdown() then
                        pcall(function() UIParent:SetScale(LUICore._pendingUIScale) end)
                        LUICore._pendingUIScale = nil
                        if LUICore.UIMult then
                            LUICore:UIMult()
                        end
                    end
                end)
            end
            LUICore._scaleRegenFrame:RegisterEvent("PLAYER_REGEN_ENABLED")
        else
    ...
    if LUICore.Minimap then
        -- Small delay to ensure profile data is fully loaded
        C_Timer.After(0.1, function()
            if LUICore.Minimap.Refresh then
    ...
    if _G.LuheyUI_RefreshSpecProfilesTab then
        _G.LuheyUI_RefreshSpecProfilesTab()
    end
...
function LUICore:ShowProfileChangeNotification()
    -- Create a simple popup frame if it doesn't exist
    if not self.profileChangePopup then
        local popup = CreateFrame("Frame", "LUICore_ProfileChangePopup", UIParent, "BackdropTemplate")
...
function LUICore:SelectEditModeElement(elementType, elementKey)
...
function LUICore:ClearEditModeSelection()
    self:HideCurrentSelectionArrows()
...
function LUICore:HideCurrentSelectionArrows()
    ...
    if sel.selectedType == "unitframe" then
        ...
        if ns.LUI_UnitFrames and ns.LUI_UnitFrames.frames then
            local frame = ns.LUI_UnitFrames.frames[sel.selectedKey]
...
function LUICore:ShowSelectionArrows(elementType, elementKey)
    if elementType == "unitframe" then
        if ns.LUI_UnitFrames and ns.LUI_UnitFrames.frames then
            local frame = ns.LUI_UnitFrames.frames[elementKey]
            if frame and frame.editOverlay then
...
function LUICore:ShowNudgeButtons(overlay)
...
function LUICore:HideNudgeButtons(overlay)
    if not overlay then return end
...
function LUICore:OnEnable()
    ...
    C_Timer.After(1.0, function()
        ...
        if _G.LuheyUI_RefreshUIHider then
            _G.LuheyUI_RefreshUIHider()
        end
...
function LUICore:OpenConfig()
    -- Open the new custom GUI instead of AceConfig
    if LuheyUI and LuheyUI.GUI then
        LuheyUI.GUI:Toggle()
    end
end

function LUICore:CreateMinimapButton()
    ...
    local dataObj = LDB:NewDataObject(ADDON_NAME, {
        type = "launcher",
        icon = "Interface\\AddOns\\LuheyUI\\assets\\luheyLogo.tga",
        label = "Luhey UI",
        ...
        OnTooltipShow = function(tooltip)
            tooltip:SetText("|cFF30D1FFLuhey UI|r")
...
function LUICore:SkinIcon(icon, settings)
...
function LUICore:SkinAllIconsInViewer(viewer)
    ...
    for _, icon in ipairs(children) do
        if IsCooldownIconFrame(icon) and (icon.icon or icon.Icon) then
            ...
            if not ok then
                ...
                print("|cffff4444[LUICore] SkinIcon error for", name, "icon:", err, "|r")
            end
        end
    end
end
...
function LUICore:ApplyViewerLayout(viewer)
    if not viewer or not viewer.GetName then return end
    local name     = viewer:GetName()
    local settings = self.db.profile.viewers[name]
    if not settings or not settings.enabled then return end
...
function LUICore:RescanViewer(viewer)
    if not viewer or not viewer.GetName then return end
    local name     = viewer:GetName()
    local settings = self.db.profile.viewers[name]
    if not settings or not settings.enabled then return end
...
function LUICore:ApplyViewerSkin(viewer)
    if not viewer or not viewer.GetName then return end
    local name     = viewer:GetName()
    local settings = self.db.profile.viewers[name]
    if not settings or not settings.enabled then return end
...
function LUICore:ProcessPendingIcons()
...
function LUICore:ForceRefreshBuffIcons()
...
function LUICore:ForceReskinAllViewers()
    for _, name in ipairs(self.viewers) do
        local viewer = _G[name]
        if viewer then
            local container = viewer.viewerFrame or viewer
        ...
        hooksecurefunc(EditModeManagerFrame, "ExitEditMode", function()
            C_Timer.After(0.1, function()
                ...
                C_Timer.After(0.15, function()
                    for _, barName in ipairs({"LuheyUIPrimaryPowerBar", "LuheyUISecondaryPowerBar"}) do
...
function LUICore:ProcessPendingBackdrops()
...
function QUI:GetGlobalFont()
    ...
    if LUICore and LUICore.db and LUICore.db.profile and LUICore.db.profile.general then
        fontName = LUICore.db.profile.general.font or fontName
    end

    return LSM:Fetch("font", fontName) or [[Interface\AddOns\LuheyUI\assets\Quazii.ttf]]
end
...
function QUI:GetGlobalTexture()
    ...
    return LSM:Fetch("statusbar", textureName) or "Interface\\AddOns\\LuheyUI\\assets\\Quazii"
end
...
function LUICore:SafeSetFont(fontString, fontPath, size, flags)
...
local QUAZII_FONT_PATH = [[Interface\AddOns\LuheyUI\assets\Quazii.ttf]]
...
local function GetGlobalFontPath()
    if not LUICore.db or not LUICore.db.profile or not LUICore.db.profile.general then
        return QUAZII_FONT_PATH
    end
    local fontName = LUICore.db.profile.general.font or "Quazii"
...
local function ScheduleGlobalFontApply()
    ...
    C_Timer.After(0.05, function()
        ...
        if LUICore.ApplyGlobalFont then
            LUICore:ApplyGlobalFont()
        end
    end)
end

function LUICore:ApplyGlobalFont()
    ...
    if not globalFontHooksInitialized then
        ...
        if ObjectiveTrackerFrame then
            if type(ObjectiveTracker_Update) == "function" then
                hooksecurefunc("ObjectiveTracker_Update", function()
                    if not LUICore.db.profile.general.applyGlobalFontToBlizzard then return end
        ...
        chatFontEventFrame:SetScript("OnEvent", function()
            if not LUICore.db or not LUICore.db.profile then return end
            if not LUICore.db.profile.general.applyGlobalFontToBlizzard then return end
            C_Timer.After(0.05, function()
                local fp = GetGlobalFontPath()
