...
local function GetSettings()
    if LUICore and LUICore.db and LUICore.db.profile and LUICore.db.profile.raidBuffs then
        return LUICore.db.profile.raidBuffs
    end
    return {
        enabled = true,
...
local function CreateMainFrame()
    ...
    mainFrame = CreateFrame("Frame", "LuheyUI_MissingRaidBuffs", UIParent)
...
function LUI_RaidBuffs:RefreshColors()
    ApplySkin()
end

_G.LuheyUI_RefreshRaidBuffColors = function()
    LUI_RaidBuffs:RefreshColors()
end
...
local function OnEvent(self, event, ...)
    ...
    elseif event == "PLAYER_REGEN_DISABLED" then
        -- Immediately hide when entering combat (bypass throttle)
        if mainFrame then mainFrame:Hide() end
    elseif event == "PLAYER_REGEN_ENABLED" then
        -- Combat ended - update display to show missing buffs again
        ThrottledUpdate()
...
function LUI_RaidBuffs:Toggle()
    local settings = GetSettings()
    settings.enabled = not settings.enabled
    UpdateDisplay()
end

function LUI_RaidBuffs:ForceUpdate()
    UpdateDisplay()
    ApplySkin()
end

function LUI_RaidBuffs:Debug()
    local settings = GetSettings()
    local lines = {}
    local playerClass = SafeUnitClass("player")
    table.insert(lines, "LUI RaidBuffs Debug")
...
SLASH_LUIRAIDBUFFS1 = "/quibuffs"
SlashCmdList["LUIRAIDBUFFS"] = function()
    if ns.RaidBuffs then
        ns.RaidBuffs:Debug()
    end
end

function LUI_RaidBuffs:GetFrame()
    return mainFrame
end

function LUI_RaidBuffs:TogglePreview()
...
function LUI_RaidBuffs:IsPreviewMode()
    return previewMode
end

-- Global function for options panel
_G.LuheyUI_ToggleRaidBuffsPreview = function()
    if ns.RaidBuffs then
