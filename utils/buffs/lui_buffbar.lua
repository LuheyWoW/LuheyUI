...
local function ApplyIconStyle(icon, settings)
    ...
    if cooldown then
        ...
        local showBuffIconSwipe = LUICore and LUICore.db and LUICore.db.profile.cooldownSwipe
            and LUICore.db.profile.cooldownSwipe.showBuffIconSwipe or false
...
local function ForcePopulateBuffIcons()
    ...
    -- Method 3: Force a rescan via LUICore if available
    if _G.LuheyUI and _G.LuheyUI.LUICore then
        local LUICore = _G.LuheyUI.LUICore
        if LUICore.ForceRefreshBuffIcons then
            C_Timer.After(0.2, function()
                pcall(function() LUICore:ForceRefreshBuffIcons() end)
            end)
        end
    end
end
...
LUI_BuffBar.LayoutIcons = LayoutBuffIcons
LUI_BuffBar.LayoutBars = LayoutBuffBars
LUI_BuffBar.Initialize = Initialize

-- Force refresh function (can be called from GUI)
function LUI_BuffBar.Refresh()
...
_G.LuheyUI_RefreshBuffBar = LUI_BuffBar.Refresh
