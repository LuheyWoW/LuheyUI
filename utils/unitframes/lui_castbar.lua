...
---------------------------------------------------------------------------
local LUI_Castbar = {}
ns.LUI_Castbar = LUI_Castbar

LUI_Castbar.castbars = {}
...
-- SETUP HELPERS
---------------------------------------------------------------------------
function LUI_Castbar:SetHelpers(helpers)
    Helpers = helpers or {}
end
...
local STAGE_COLORS = LUI_Castbar.STAGE_COLORS
local STAGE_FILL_COLORS = LUI_Castbar.STAGE_FILL_COLORS
...
function LUI_Castbar:GetEmpoweredLevel()
    local playerCastbar = self.castbars["player"]
    if not playerCastbar then
        playerCastbar = _G.LuheyUI_Castbars and _G.LuheyUI_Castbars["player"]
    end
...
---------------------------------------------------------------------------
function LUI_Castbar:CreateCastbar(unitFrame, unit, unitKey)
    local settings = GetUnitSettings(unitKey)
...
---------------------------------------------------------------------------
function LUI_Castbar:SetupCastbar(castbar, unit, unitKey, castSettings)
        ...
        if spellName or channelName or isInEmpoweredHold then
            ...
            if isPlayer and self.isEmpowered then
                ...
                if self.empoweredLevelText and self.showEmpoweredLevel then
                    local currentStage, maxStages, isEmpowered = LUI_Castbar:GetEmpoweredLevel()
...
function LUI_Castbar:SetupTargetFocusCastbar(castbar, unit, unitKey, castSettings)
    self:SetupCastbar(castbar, unit, unitKey, castSettings)
end

function LUI_Castbar:SetupPlayerCastbar(castbar, unit, unitKey, castSettings)
    self:SetupCastbar(castbar, unit, unitKey, castSettings)
end
...
-- BOSS CASTBAR SETUP
---------------------------------------------------------------------------
function LUI_Castbar:SetupBossCastbar(castbar, unit, bossIndex, castSettings)
    -- Unified OnUpdate handler - handles both real casts and preview
    local function CastBar_OnUpdate(self, elapsed)
        -- Check if actually casting (real cast takes priority)
        local spellName = UnitCastingInfo(self.unit)
        ...
        if spellName and startTimeMS and endTimeMS then
            ...
            C_Timer.After(0.1, function()
                if not UnitCastingInfo(self.unit) and not UnitChannelInfo(self.unit) then
                    ...
                    local LUI_UF = LUI_Castbar.unitFramesModule
                    local bossFramePreviewActive = LUI_UF and LUI_UF.previewMode and LUI_UF.previewMode["boss" .. bossIndex]
...
-- CREATE: Boss Castbar
---------------------------------------------------------------------------
function LUI_Castbar:CreateBossCastbar(unitFrame, unit, bossIndex)
    ...
    local anchorFrame = CreateAnchorFrame("LUI_Boss" .. bossIndex .. "_Castbar", UIParent)
    anchorFrame:SetSize(castWidth, barHeight)
        ...
        if spellName and startTimeMS and endTimeMS then
            ...
            C_Timer.After(0.1, function()
                if not UnitCastingInfo(self.unit) and not UnitChannelInfo(self.unit) then
                    local settings = GetUnitSettings(self.unitKey)
                    local LUI_UF = LUI_Castbar.unitFramesModule
                    local bossFramePreviewActive = LUI_UF and LUI_UF.previewMode and LUI_UF.previewMode["boss" .. self.bossIndex]
                    if settings and settings.castbar and settings.castbar.previewMode and bossFramePreviewActive then
                        -- Show preview simulation
...
-- GLOBAL FUNCTIONS
---------------------------------------------------------------------------
LUI_Castbar.unitFramesModule = nil

function LUI_Castbar:SetUnitFramesModule(ufModule)
...
_G.LuheyUI_ShowCastbarPreview = function(unitKey)
    local settings = GetUnitSettings(unitKey)
    ...
    local LUI_UF = LUI_Castbar.unitFramesModule
    if LUI_UF then
        LUI_UF:RefreshFrame(unitKey)
    end
end

_G.LuheyUI_HideCastbarPreview = function(unitKey)
    local settings = GetUnitSettings(unitKey)
    if not settings or not settings.castbar then
        return
    end
...
---------------------------------------------------------------------------
function LUI_Castbar:RefreshCastbar(castbar, unitKey, castSettings, unitFrame)
    ...
    if newCastbar then
        local LUI_UF = self.unitFramesModule
        if LUI_UF and LUI_UF.castbars then
            LUI_UF.castbars[unitKey] = newCastbar
        end
    end
end

function LUI_Castbar:RefreshBossCastbar(castbar, bossKey, castSettings, unitFrame)
    if not castSettings or not unitFrame then return end
    ...
    if newCastbar then
        local LUI_UF = self.unitFramesModule
        if LUI_UF and LUI_UF.castbars then
            LUI_UF.castbars[bossKey] = newCastbar
        end
    end
end

_G.LuheyUI_RefreshCastbar = function(unitKey)
    local LUI_UF = LUI_Castbar.unitFramesModule
    if not LUI_UF then return end
    LUI_UF:RefreshFrame(unitKey)
end

-- Refresh all castbars (used by HUD Layering options)
_G.LuheyUI_RefreshCastbars = function()
    local LUI_UF = LUI_Castbar.unitFramesModule
    if not LUI_UF then return end
...
_G.LuheyUI_Castbars = LUI_Castbar.castbars
