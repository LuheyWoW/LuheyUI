...
---------------------------------------------------------------------------
local LUI_UF = {}
ns.LUI_UnitFrames = LUI_UF

-- Frame references
LUI_UF.frames = {}
LUI_UF.castbars = {}
LUI_UF.previewMode = {}
LUI_UF.auraPreviewMode = {}  -- Tracks buff/debuff preview state (keyed by "unitKey_buff" or "unitKey_debuff")

-- Reference to castbar module
local LUI_Castbar = ns.LUI_Castbar

-- Edit Mode: Slider registry for real-time sync during drag
-- Format: { unitKey = { x = sliderRef, y = sliderRef, coordText = fontStringRef } }
LUI_UF.editModeSliders = {}
...
local function GetDB()
    if LUICore and LUICore.db and LUICore.db.profile and LUICore.db.profile.quiUnitFrames then
        return LUICore.db.profile.quiUnitFrames
    end
    return nil
end
...
local function GetGeneralSettings()
    ...
    if LUICore and LUICore.db and LUICore.db.profile and LUICore.db.profile.general then
        return LUICore.db.profile.general
    end
    return nil
end
...
---------------------------------------------------------------------------
-- HELPER: Pixel-perfect scaling (uses LUICore:Scale if available)
---------------------------------------------------------------------------
local function Scale(x)
    if LUICore and LUICore.Scale then
        return LUICore:Scale(x)
    end
    return x
end
...
local function GetAbsorbTexturePath(textureName)
    ...
    return LSM:Fetch("statusbar", name) or "Interface\\AddOns\\LuheyUI\\assets\\absorb_stripe"
end
...
local function CreateBossFrame(unit, frameKey, bossIndex)
    ...
    -- Create secure button with unique name (LUI_Boss1, LUI_Boss2, etc.)
    local frameName = "LUI_Boss" .. bossIndex
    ...
    frame:HookScript("OnShow", function(self)
        ...
        if LUI_UF.previewMode[bossKey] then return end
        UpdateFrame(self)
    end)
...
local function ForceUpdateToT()
    local totFrame = LUI_UF.frames and LUI_UF.frames.targettarget
...
local function CreateUnitFrame(unit, unitKey)
    ...
    local frameName = "LUI_" .. unitKey:gsub("^%l", string.upper)
    local frame = CreateFrame("Button", frameName, UIParent, "SecureUnitButtonTemplate, BackdropTemplate, PingableUnitFrameTemplate")
...
local function CreateCastbar(unitFrame, unit, unitKey)
    ...
    if LUI_Castbar then
        return LUI_Castbar:CreateCastbar(unitFrame, unit, unitKey)
    end
    return nil
end
...
local function CreateBossCastbar(unitFrame, unit, bossIndex)
    -- Delegate to castbar module
    if LUI_Castbar then
        return LUI_Castbar:CreateBossCastbar(unitFrame, unit, bossIndex)
    end
    return nil
end
...
local function UpdateAuras(frame)
    ...
    local buffPreviewActive = LUI_UF.auraPreviewMode[unitKey .. "_buff"]
    local debuffPreviewActive = LUI_UF.auraPreviewMode[unitKey .. "_debuff"]
...
---------------------------------------------------------------------------
function LUI_UF:ShowPreview(unitKey)
        ...
        for i = 1, 5 do
            ...
            if frame then
                ...
                if settings and settings.castbar and settings.castbar.previewMode then
                    ...
                    if castbar and LUI_Castbar then
                        -- Trigger the castbar to show its preview
                        LUI_Castbar:RefreshBossCastbar(castbar, bossKey, settings.castbar, frame)
                    end
                end
...
function LUI_UF:HidePreview(unitKey)
...
---------------------------------------------------------------------------
function LUI_UF:ShowAuraPreview(unitKey, auraType)
...
function LUI_UF:ShowAuraPreviewForFrame(frame, unitKey, auraType)
    if not frame then return end
...
function LUI_UF:HideAuraPreview(unitKey, auraType)
...
function LUI_UF:HideAuraPreviewForFrame(frame, unitKey, auraType)
    if not frame then return end
...
---------------------------------------------------------------------------
function LUI_UF:RefreshFrame(unitKey)
    ...
    if unitKey == "boss" then
        ...
        local hudLayering = LUICore and LUICore.db and LUICore.db.profile and LUICore.db.profile.hudLayering
        local bossLayerPriority = hudLayering and hudLayering.bossFrames or 4
        local bossFrameLevel
        if LUICore and LUICore.GetHUDFrameLevel then
            bossFrameLevel = LUICore:GetHUDFrameLevel(bossLayerPriority)
        end
        ...
        for i = 1, 5 do
            ...
            if frame then
                ...
                local castbar = self.castbars[bossKey]
                if castbar and LUI_Castbar and LUI_Castbar.RefreshBossCastbar then
                    local castSettings = settings.castbar
                    if castSettings then
                        LUI_Castbar:RefreshBossCastbar(castbar, bossKey, castSettings, frame)
                    end
                end
            end
        end
    ...
    if LUICore and LUICore.GetHUDFrameLevel then
        local frameLevel = LUICore:GetHUDFrameLevel(layerPriority)
    ...
    if unitKey == "player" and settings.indicators then
        ...
        if frame.indicatorFrame then
            ...
            if LUICore and LUICore.GetHUDFrameLevel then
                local indicatorLevel = LUICore:GetHUDFrameLevel(indicatorPriority)
    ...
    if castSettings and castSettings.enabled then
        if castbar and LUI_Castbar and LUI_Castbar.RefreshCastbar then
            -- Castbar exists - refresh it
            LUI_Castbar:RefreshCastbar(castbar, unitKey, castSettings, frame)
        elseif not castbar and LUI_Castbar and LUI_Castbar.CreateCastbar then
            -- Castbar doesn't exist but is now enabled - create it
            self.castbars[unitKey] = LUI_Castbar:CreateCastbar(frame, unitKey, unitKey)
        end
    end
end

function LUI_UF:RefreshAll()
...
---------------------------------------------------------------------------
LUI_UF.editModeActive = false

-- Register slider references for real-time sync during edit mode
function LUI_UF:RegisterEditModeSliders(unitKey, xSlider, ySlider)
...
function LUI_UF:NotifyPositionChanged(unitKey, offsetX, offsetY)
...
local function CreateNudgeButton(parent, direction, deltaX, deltaY, unitKey)
    ...
    btn:SetScript("OnClick", function()
        ...
        if settings then
            ...
            LUI_UF:NotifyPositionChanged(settingsKey, settings.offsetX, settings.offsetY)
        end
    end)
...
function LUI_UF:EnableEditMode()
    if InCombatLockdown() then
        print("|cFF56D1FFLuheyUI|r: Cannot enter Edit Mode during combat.")
        return
    end
    ...
    if not self.exitEditModeBtn then
        local exitBtn = CreateFrame("Button", "LUI_ExitEditModeBtn", UIParent, "BackdropTemplate")
    ...
    for unitKey, frame in pairs(self.frames) do
        ...
        frame:SetScript("OnMouseDown", function(self, button)
            if button == "LeftButton" and LUI_UF.editModeActive then
                local LUICore = _G.LuheyUI and _G.LuheyUI.LUICore
                if LUICore and LUICore.SelectEditModeElement then
                    LUICore:SelectEditModeElement("unitframe", self._editModeUnitKey)
                end
            end
        end)

        frame:SetScript("OnDragStart", function(self)
            if LUI_UF.editModeActive then
                ...
                self:SetScript("OnUpdate", function(self)
                    ...
                    if selfX and selfY and parentX and parentY then
                        ...
                        if settings then
                            ...
                            LUI_UF:NotifyPositionChanged(settingsKey, offsetX, offsetY)
                        end
                    end
                end)
            end
        ...
        frame:SetScript("OnKeyDown", function(self, key)
            if not LUI_UF.editModeActive then return end
    ...
    print("|cFF56D1FFLuheyUI|r: Edit Mode |cff00ff00ENABLED|r - Drag frames to reposition.")
end

function LUI_UF:DisableEditMode()
    self.editModeActive = false

    -- Clear Edit Mode selection (hides arrows on selected element)
    local LUICore = _G.LuheyUI and _G.LuheyUI.LUICore
    if LUICore and LUICore.ClearEditModeSelection then
        LUICore:ClearEditModeSelection()
    end
    ...
    print("|cFF56D1FFLuheyUI|r: Edit Mode |cffff0000DISABLED|r - Positions saved.")
end

function LUI_UF:ToggleEditMode()
...
function LUI_UF:HideBlizzardFrames()
    local db = GetDB()
...
function LUI_UF:Initialize()
    ...
    if LUI_Castbar then
        ...
        LUI_Castbar:SetUnitFramesModule(self)
        LUI_Castbar.castbars = self.castbars
    end
...
function LUI_UF:HookBlizzardEditMode()
...
function LUI_UF:HideBlizzardSelectionFrames()
...
---------------------------------------------------------------------------
function LUI_UF:RegisterWithClique()
...
initFrame:SetScript("OnEvent", function(self, event)
    if event == "PLAYER_LOGIN" then
        ...
        C_Timer.After(0.5, function()
            ...
            LUI_UF:HookBlizzardEditMode()
            -- Register frames with Clique after creation
            C_Timer.After(0.5, function()
                LUI_UF:RegisterWithClique()
            end)
        end)
    ...
    elseif event == "PLAYER_REGEN_ENABLED" then
        ...
        if LUI_UF.pendingInitialize then
            LUI_UF.pendingInitialize = false
            LUI_UF:Initialize()
            LUI_UF:HookBlizzardEditMode()
            C_Timer.After(0.5, function()
                LUI_UF:RegisterWithClique()
...
_G.LuheyUI_RefreshAuras = function(unitKey)
    if unitKey then
        ...
        if unitKey == "boss" then
            ...
            local frame = LUI_UF.frames[unitKey]
            if frame then
                lastAuraUpdate[unitKey] = 0
                UpdateAuras(frame)
            end
...
_G.LuheyUI_ShowUnitFramePreview = function(unitKey)
    LUI_UF:ShowPreview(unitKey)
end

_G.LuheyUI_HideUnitFramePreview = function(unitKey)
    LUI_UF:HidePreview(unitKey)
end

_G.LuheyUI_ShowAuraPreview = function(unitKey, auraType)
    LUI_UF:ShowAuraPreview(unitKey, auraType)
end

_G.LuheyUI_HideAuraPreview = function(unitKey, auraType)
    LUI_UF:HideAuraPreview(unitKey, auraType)
end

_G.LuheyUI_ToggleUnitFrameEditMode = function()
    LUI_UF:ToggleEditMode()
end

-- Register slider references for real-time sync during edit mode
_G.LuheyUI_RegisterEditModeSliders = function(unitKey, xSlider, ySlider)
    LUI_UF:RegisterEditModeSliders(unitKey, xSlider, ySlider)
end

-- Global references for external access
_G.LuheyUI_UnitFrames = LUI_UF.frames
_G.LuheyUI_Castbars = LUI_UF.castbars
...
local function GetAnchorFrame(anchorType)
    ...
    elseif anchorType == "primary" then
        local LUICore = _G.LuheyUI and _G.LuheyUI.LUICore
        return LUICore and LUICore.powerBar
    elseif anchorType == "secondary" then
        local LUICore = _G.LuheyUI and _G.LuheyUI.LUICore
        return LUICore and LUICore.secondaryPowerBar
    end
    return nil
end
...
_G.LuheyUI_UpdateAnchoredUnitFrames = function()
    ...
    if playerAnchorType and playerAnchorType ~= "disabled" and LUI_UF.frames.player then
        ...
        if anchorFrame and anchorFrame:IsShown() then
            ...
            if anchor then
                local frame = LUI_UF.frames.player
    ...
    if targetAnchorType and targetAnchorType ~= "disabled" and LUI_UF.frames.target then
        local anchorFrame = GetAnchorFrame(targetAnchorType)
        if anchorFrame and anchorFrame:IsShown() then
            local anchor = GetAnchorDimensions(anchorFrame, targetAnchorType)
            if anchor then
                local frame = LUI_UF.frames.target
                local frameWidth = frame:GetWidth()
                local frameHeight = frame:GetHeight()
...
_G.LuheyUI_UpdateCDMAnchoredUnitFrames = _G.LuheyUI_UpdateAnchoredUnitFrames
...
_G.LuheyUI_UpdateLockedCastbarToEssential = function(forceUpdate)
    local db = GetDB()
    ...
    if _G.LuheyUI_RefreshCastbar then
        _G.LuheyUI_RefreshCastbar("player")
    end
end

-- Global callback for NCDM to update castbar anchored to Utility
-- Width is now controlled by dual anchors, so just refresh the castbar
_G.LuheyUI_UpdateLockedCastbarToUtility = function(forceUpdate)
    local db = GetDB()
    if not db or not db.player then return end
...
_G.LuheyUI_UpdateLockedCastbarToFrame = function()
    local db = GetDB()
    if not db or not db.player then return end
