...
-- Combat deferral flag
GUI.pendingOpen = false
...
---------------------------------------------------------------------------
local FONT_PATH = LSM:Fetch("font", "Quazii") or [[Interface\AddOns\LuheyUI\assets\Quazii.ttf]]
...
function GUI:ShowConfirmation(options)
    ...
    if not confirmDialog then
        ...
        confirmDialog = CreateFrame("Frame", "LUI_ConfirmDialog", UIParent, "BackdropTemplate")
...
function GUI:CreateFormDropdown(parent, label, options, dbKey, dbTable, onChange, registryInfo)
    ...
    dropdown:SetScript("OnClick", function()
        if menuFrame:IsShown() then
            ...
            -- Check for forced upward flag or auto-detect overflow
            local openUpward = menuFrame.openUpward
            if not openUpward then
                -- Auto-detect: check if menu would overflow bottom of screen
                local menuHeight = menuFrame:GetHeight() or 0
                local dropdownBottom = dropdown:GetBottom()
                local buffer = 20
                if dropdownBottom and (dropdownBottom - menuHeight - buffer) < 0 then
                    openUpward = true
                end
            end
...
function GUI:CreateMainFrame()
    ...
    local savedWidth = QUI.LUICore and QUI.LUICore.db and QUI.LUICore.db.profile.configPanelWidth or FRAME_WIDTH
    ...
    local frame = CreateFrame("Frame", "LuheyUI_Options", UIParent, "BackdropTemplate")
    ...
    local savedAlpha = QUI.LUICore and QUI.LUICore.db and QUI.LUICore.db.profile.configPanelAlpha or 0.97
    ...
    -- Auto-close on combat start, auto-open on combat end if pending
    ...
    title:SetText("Luhey UI (Originally by Quazii)")
    ...
    version:SetText("Version " .. (LuheyUI.versionString or "2.0.0"))
    ...
    local function ApplyScale(value)
        ...
        if QUI.LUICore and QUI.LUICore.db then
            QUI.LUICore.db.profile.configPanelScale = value
        end
        return value
    end

    -- Initialize scale from saved value
    local savedScale = QUI.LUICore and QUI.LUICore.db and QUI.LUICore.db.profile.configPanelScale or 1.0
    ...
    resizeHandle:SetScript("OnMouseUp", function(self, button)
        if button == "LeftButton" then
            ...
            if QUI.LUICore and QUI.LUICore.db then
                QUI.LUICore.db.profile.configPanelWidth = frame:GetWidth()
            end
        end
    end)
...
function GUI:Show()
    if InCombatLockdown() then
        ...
        print("|cFF56D1FFLuheyUI:|r Settings will open after combat ends.")
        return
    end
