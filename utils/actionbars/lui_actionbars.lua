...
local TEXTURE_PATH = [[Interface\AddOns\LuheyUI\assets\iconskin\]]
local TEXTURES = {
...
local function GetDB()
    ...
    return LUICore.db.profile.actionBars
end
...
local function GetExtraButtonDB(buttonType)
    local LUICore = _G.LuheyUI and _G.LuheyUI.LUICore
    if not LUICore or not LUICore.db or not LUICore.db.profile then return nil end
    return LUICore.db.profile.actionBars and LUICore.db.profile.actionBars.bars
        and LUICore.db.profile.actionBars.bars[buttonType]
end
...
local function CreateExtraButtonHolder(buttonType, displayName)
    ...
    -- Create holder frame
    local holder = CreateFrame("Frame", "LUI_" .. buttonType .. "Holder", UIParent)
    ...
    local mover = CreateFrame("Frame", "LUI_" .. buttonType .. "Mover", holder, "BackdropTemplate")
...
_G.LuheyUI_ToggleExtraButtonMovers = ToggleExtraButtonMovers
_G.LuheyUI_RefreshExtraButtons = RefreshExtraButtons
...
local function ApplyPageArrowVisibility(hide)
    ...
    if hide then
        pageNum:Hide()
        if not pageNum._LUI_ShowHooked then
            pageNum._LUI_ShowHooked = true
...
_G.LuheyUI_ApplyPageArrowVisibility = ApplyPageArrowVisibility
...
_G.LuheyUI_RefreshActionBars = function()
    if InCombatLockdown() then
