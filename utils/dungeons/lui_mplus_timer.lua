...
local function GetSettings()
    ...
    if LUICore and LUICore.db and LUICore.db.profile and LUICore.db.profile.mplusTimer then
        return LUICore.db.profile.mplusTimer
    end
    return {
        enabled = true,
...
local function GetPosition()
    ...
    local LUICore = _G.LuheyUI and _G.LuheyUI.LUICore
    if LUICore and LUICore.db and LUICore.db.profile and LUICore.db.profile.mplusTimer then
        local pos = LUICore.db.profile.mplusTimer.position
        if pos then
            return {
...
local function SavePosition(point, relPoint, x, y)
    ...
    if LUICore and LUICore.db and LUICore.db.profile then
        if not LUICore.db.profile.mplusTimer then
            LUICore.db.profile.mplusTimer = {}
        end
        LUICore.db.profile.mplusTimer.position = {
            point = point,
            relPoint = relPoint,
            x = x,
            y = y
...
function MPlusTimer:CreateFrames()
    ...
    -- Root frame
    local root = CreateFrame("Frame", "LUI_MPlusTimerFrame", UIParent)
...
-- Slash Command
---------------------------------------------------------------------------
SLASH_LUIMPLUSTIMER1 = "/qmpt"
SlashCmdList["LUIMPLUSTIMER"] = function(msg)
...
-- Expose for skinning
---------------------------------------------------------------------------
_G.LuheyUI_MPlusTimer = MPlusTimer
