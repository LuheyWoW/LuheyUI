...
local function GetDungeonInfo(mapID)
    ...
    if _G.LUI_DungeonData then
        local data = _G.LUI_DungeonData.GetDungeonData(mapID)
        if data then
...
local function GetKeyColor(level)
    ...
    if _G.LUI_DungeonData and _G.LUI_DungeonData.GetKeyColor then
        return _G.LUI_DungeonData.GetKeyColor(level)
    end
    -- Fallback
...
local KeyTrackerFrame = CreateFrame("Frame", "LUIKeyTrackerFrame", UIParent, "BackdropTemplate")
...
_G.LuheyUI_RefreshKeyTrackerColors = ApplySkinColors
...
_G.LuheyUI_RefreshKeyTrackerFonts = UpdateAllButtonFonts
...
KeyTrackerFrame:SetScript("OnMouseUp", function(self, button)
    if button == "RightButton" and not InCombatLockdown() then
        RequestKeystones()
        print("|cFF56D1FFLuheyUI:|r Refreshing party keys...")
    end
end)
